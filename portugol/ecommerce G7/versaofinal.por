		...
		funcao inicio()
		{
			...
			*/		
			...
			escolha (opcaoPagamento) {
				...
				caso 1: 
				escreva ("\n - A VISTA COM 10% DESCONTO -> " + math.arredondar(valorTotal * 0.9, 2))
				pare
				
				caso 2:
				escreva ("\n - NO CARTAO COM ACRESCIMO DE 10% -> " + math.arredondar(valorTotal * 1.1, 2))
				pare
				
				caso 3:
				escreva ("\n - 2X COM ACRESCIMO DE 15% -> " + math.arredondar(valorTotal * 1.15, 2) + " - VALOR DE CADA PARCELA: " + (valorTotal * 1.15)/2)
				pare
			}
				...
				escreva("\nCOMPRA REALIZADA COM SUCESSO! \n DESEJA A EMISSAO DA NOTA FISCAL (S/N)?")
				leia (notaFiscal)
				notaFiscal = Texto.caixa_alta(notaFiscal)
				se (notaFiscal=="S"){
